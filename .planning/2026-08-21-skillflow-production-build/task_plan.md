...
## Current Phase
Gate E — creator foundation; user authorized continuation on 2026-08-27 after Gate D Sites version 9 deployment
...
5. **Gate E — creator foundation (local acceptance PASS; awaiting user checkpoint):** submit/import/create/fork draft, publisher declaration, append-only Revision, conversational structured Diff, E1/E2 evidence, atomic immutable Release, same-Skill next version, pending source claim, Registry discovery/download and Gate C exact binding are implemented. The migration is additive and preserves existing Release references; publication uses live-lease plus current-head fences; SemVer must advance; old Release presentation is manifest-owned; exact Fork reads the immutable R2 artifact; failed source storage is idempotently recoverable with end-to-end attempt fences and no shared-key cleanup race. Production build, lint, diff-check, 96/96 tests and extended local D1/R2 HTTP smoke pass. Final independent Red Team verdict is PASS with P0=0, P1=0 and P2=0. No commit, push or deployment has occurred.

## Gate E Acceptance Contract (frozen implementation target)

1. Creator actions require an authenticated Personal Workspace. A draft is owned and queried by server-derived workspace/account identity; every object route scopes the object and workspace in the same query and returns a non-enumerating 404 across workspaces.
2. MVP inputs are bounded UTF-8 Skill text (`SKILL.md` paste/upload), a natural-language creation brief, and an authoritative fork from an existing Registry Release. Public Git repository URLs may be recorded as attribution, but the server does not crawl arbitrary URLs, follow redirects or execute imported files. Arbitrary third-party scripts remain non-executable.
3. Parsing produces one canonical draft projection plus append-only DraftRevision history: original source text/digest, name, Chinese brief, description, task tags, inputs/outputs, permission declarations, attribution, license evidence, limitations and execution policy. System inference is visibly separate from creator-confirmed facts; edits use optimistic revision CAS, persist a structured Diff and preserve source attribution.
4. E1 is a deterministic structural/safety/license review tied to the exact draft revision and content digest. It checks required fields, size/encoding, unsafe executable or secret patterns, permissions, unverifiable claims, source/license evidence and publication eligibility. A stale evaluation cannot authorize publication.
5. E2 is an isolated instruction-only sample run with no connectors, filesystem, network tools or script execution. It is optional for an E1-labelled release, required only to claim tested outcome evidence, and cannot run without configured server-side model routing. The UI must never turn an unexecuted sample into a passing result.
6. Publishing is one server transaction/CAS from the exact reviewed draft revision. It creates a new Skill plus immutable SkillRelease snapshot, records creator/source/license/evaluation provenance and makes the release discoverable only after commit. Database triggers prevent material update or deletion after publication; status may later become yanked/revoked. Changes create a new draft/release version.
7. Creator ownership means “published from this authenticated workspace”, not proof of upstream authorship. Upstream author claims use a separate pending verification state and may not set a verified badge from user assertion alone. Internal commercial-use classification remains operator-only and is not shown publicly in the free beta.
8. Every creator state is backend-owned and durable in D1; original uploaded/pasted bytes are stored as an opaque R2 snapshot when available. D1/R2 use pending-to-ready commit semantics. UI may not claim saved, tested, published, verified, downloadable or revenue-bearing from local state alone.
9. Published creator releases enter the existing Registry with a source namespace, Chinese brief, original attribution, license, permissions, limitations, E1/E2 evidence and honest hosted-execution status. Same-slug upstream and creator Skills must not collide. A creator Release must be authoritatively resolvable and bindable in Gate C, while publication never makes third-party code executable or bypasses the Gate D runtime allowlist.
10. Gate E acceptance requires migration review, draft/revision/idempotency/access-control tests, malicious-input and stale-evaluation tests, publication atomicity and immutable-release tests, Registry discovery tests, responsive/keyboard/IME checks, lint/build/diff-check, a local D1/R2 HTTP smoke and final independent Red Team PASS with no P0/P1 blockers.

### Gate E Frozen Boundaries

- No payments, revenue balance, creator payout, public author verification automation, arbitrary repository crawling, package installation, arbitrary code execution, external connector use, scheduled runs or public moderation queue in this Gate.
- A public Release may honestly carry E1 structural evidence only. E2 adds tested-sample evidence only after a real isolated model run; neither level is a guarantee of fitness for every user task.
- Natural-language draft generation uses the existing server-only Model Gateway and fails honestly when no provider is configured. Manual Skill text creation and editing remain available without a model.
- Public GitHub or source URLs are attribution fields in this Gate. Repository ownership and upstream-author claims remain pending until a later operator verification workflow.
- Registry detail and Gate C selectors use `(source, slug)` identity. A bare slug never silently changes from OpenAgentSkill to a creator Release or vice versa.
- Existing Registry entries expose “按我的需求修改” as a fork entry. AI proposes a structured Draft Diff; it never rewrites the upstream Release or authorship fields.

### Gate E Schema and API Plan (frozen before code changes)

- Add `creator_submissions` as the aggregate root and current-head projection with workspace/account ownership, input kind, current revision ID/state, source attribution, original R2 snapshot metadata, canonical draft, content digest, license/risk review snapshots and timestamps.
- Add append-only `creator_submission_revisions` with parent, mutation kind, exact snapshot/digest and server-computed structured Diff. Evaluations and publication pin the revision ID, never only a mutable submission row.
- Add append-only `creator_evaluations` keyed by submission, revision, digest and level (`e1`/`e2`); the latest passing evaluation is never inferred from mutable client state.
- Add `creator_claims` for pending/verified/rejected upstream ownership evidence without conflating a workspace-created Skill with verified upstream authorship.
- Reuse existing `skills` and `skill_releases` for committed publication. Publication records exact submission/evaluation provenance inside the immutable manifest and uses existing `audit_events` for create/edit/evaluate/publish transitions.
- Replace the unsafe global `artifact_digest` uniqueness with `(skill_id, artifact_digest)` uniqueness; cross-Skill duplicates become an audit/review signal rather than a digest-reservation denial of service. Add database triggers that reject material update or deletion after `published_at` is set.
- API sequence: authenticated list/create draft → authenticated read/update exact revision → E1 review → optional E2 isolated sample → publish exact reviewed revision → creator history/release read. Registry search/detail merges only committed public creator Releases with upstream supply.
